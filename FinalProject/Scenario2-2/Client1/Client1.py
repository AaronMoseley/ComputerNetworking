...
def primary(conn: socket)->None:
    ...
    while fileNames.lower() != "quit":
        outputFileName = "output" + fileNames.split()[0][fileNames.split()[0].index('.'):]
        ...
        f = open(os.path.join(sys.path[0], outputFileName), "wb")
        ...
        print(f"Received {fileNames}")

        downloadRate = os.path.getsize(os.path.join(sys.path[0], outputFileName)) / (downloadEnd - downloadStart)
        ...
        print(f"Output merged file to {outputFileName}")
